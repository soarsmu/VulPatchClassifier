...
def read_feature_list(file_path_list):
    url_to_feature = {}
...
def predict_test_data(model, testing_generator, device, need_prob=False):
    ...
    with torch.no_grad():
        model.eval()
        for ids, url_batch, feature_batch, label_batch in tqdm(testing_generator):
            feature_batch, label_batch = feature_batch.to(device), label_batch.to(device)
            outs = model(feature_batch)
...
def train(model, learning_rate, number_of_epochs, training_generator, test_java_generator, test_python_generator):
    ...
    for epoch in range(number_of_epochs):
        ...
        total_loss = 0
        current_batch = 0
        for id_batch, url_batch, feature_batch, label_batch in training_generator:
            feature_batch, label_batch = feature_batch.to(device), label_batch.to(device)
            outs = model(feature_batch)
